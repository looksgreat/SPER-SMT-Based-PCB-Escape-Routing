...
class PCB{
public:
    PCB(): solver(new SOLVER), tmplen(solver->getVector()), boundingBoxMode(0){}
    ...
    void BFS(vector<vector<vector<int>>> &result);
...
private:
    ...
    int mode, boundingBoxMode;
    vector<pair<pair<int, int>, int>> *pinVect, *fanoutVect;
